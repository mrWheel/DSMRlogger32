...
cutoutsLid =  [
     ...
     ,[4, 40, 6, 6, 0, yappRectangle, yappCenter]       // NeoPixel 3
...
//----------------------------------------------------------------------------
module insideSwitch(x, y, s, topPcb)
{
                      //          <-outside-->
                      //           >-------< -- (s)
  rX      = pcbX+x;   //        -+-+       +-+--  ^
  ...
  translate([rX, rY, (topPcb*-1)-0.5])
  {
    difference()
    {
      union()
      {
        ...
        {
          color("red") //cube([outside,outside, bH]);
          roundedCube([outside,outside, bH], false, radius = 1.0, apply_to = "");
        }
        //translate([(inside+wall)/-2,((inside+wall)/-2),3])
        translate([0, 0, 3])
        {
          //color("gray") cube([inside+wall, inside+wall, (topPcb-5)]);
      ...
      //translate([(inside/-2),(inside/-2),-1]) 
      translate([0, 0, 15/2]) 
      {
        //color("orange") cube([inside,inside,15]);
...
module lidHookInside()
{
  ...
  // (2) = width
  // (3) = z-ax
  insideSwitch(14, 6, 8, topPCB);
  
  //-- Flash Button
  //-- [41,  6,  8, 8, 0, yappRectangle, yappCenter]     // Flash Button
  insideSwitch(41, 6, 8, topPCB);
  ...
  /**
  translate([pcbX+4.4, pcbY+28, (topPCB/-2)])
  {
    difference()
    {
      color("red") cube([10,10,topPCB], true);
      translate([0,0,-0.6]) color("blue") cube([6,6,topPCB], true);
    }
  }
  */
  ...
  /*
  translate([pcbX+4.4, pcbY+40, (topPCB/-2)])
  {
    difference()
    {
      color("red") cube([10,10,topPCB], true);
...
if (printSwitchExtenders)
{
  ...
  translate([-10,40,0])
  {
    //color("black") cube([1,2,15]);  // above shell
  }
    ...
    translate([-15,110,0])
    {
      difference()
      {
        translate([0,0,(capHeight/2)]) color("red")
            roundedCube([capRib, capRib, capHeight], true, radius = 2.0, apply_to = "");
