...
#define RST_PIN -1
// #define PROG_VERSION "v5.0.5 (24-04-2023)"
#define _SPIFFS
...
#define writeToSysLog(...) ({                                                                                                                                                                                                                                                    \
  sysLog.writeDbg(sysLog.buildD("[%04d-%02d-%02d %02d:%02d:%02d][%-12.12s] ", localtime(&now)->tm_year + 1900, localtime(&now)->tm_mon + 1, localtime(&now)->tm_mday, localtime(&now)->tm_hour, localtime(&now)->tm_min, localtime(&now)->tm_sec, __FUNCTION__), ##__VA_ARGS__); \
})
...
extern const char* PROG_VERSION; //-- from DSMRlogger32.cpp
//-- used in DSMRlogger32.cpp
